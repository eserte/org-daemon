...
EOF
	...
	ok -e "$dir/test.ics~", "backup file will be generated by default";
	my $ics_contents = slurp "$dir/test.ics";
...
EOF
    ...
    {
	unlink "$dir/test.ics~";
	my $success = run [@full_script, '--debug', '--domain-id=example.org', "--todo-file=$dir/test.org", "--ics-file=$dir/test.ics", "--no-out-ics-backup"], '>', \my $stdout, '2>', \my $stderr;
	ok $success, 'run on existing .ics file, changed date';
	ok ! -e "$dir/test.ics~", "no backup file was created by option";
	my $ics_contents = slurp "$dir/test.ics";
